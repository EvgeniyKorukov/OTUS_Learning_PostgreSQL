...
### [Подготовка и настройка существующего экземпляра `PostgreSQL 15` на `pg-srv1`, чтобы перевести его на `Patroni`](https://patroni.readthedocs.io/en/latest/existing_data.html).
  ...
  * Создаем/изменяем пользователей с правами
    ...
    ubuntu@pg-srv1:~$
    ```
  * Правим `pg_hba.conf` и применяем изменения
    ```bash
    ...
    ```
    ```console
    ubuntu@pg-srv1:~$ sudo cat /etc/postgresql/15/main/pg_hba.conf
    ...
    ubuntu@pg-srv1:~$ sudo -u postgres psql -c "SELECT pg_reload_conf()"
     pg_reload_conf 
    ----------------
     t
    (1 row)
